...
L.TrackPlayer = class {
    ...
    addTo(map) {
        ...
        let path = this.track.geometry.coordinates.map(([lng, lat]) => [
            lat,
            lng,
        ]);
    ...
    startAction() {
        ...
        let player = (timestamp) => {
            if (timestamp && this.addedToMap) {
                let totalDuration =
                    (distance / this.options.speed) * 3600 * 1000; // 总体播放时间（毫秒）
                this.startTimestamp ||= timestamp; //为播放开始时的时间戳赋值
                let takeTime = timestamp - this.startTimestamp; //从播放开始到此刻时间过去了多久
                this.walkedDistance =
                    distance * (takeTime / totalDuration) +
                    this.walkedDistanceTemp; //根据当前时间在整体时间的占比计算到下一个点位要前进多远距离
                this.playAction();
            }
    ...
    playAction(settingProgress = false) {
        ...
        if (this.walkedDistance > 0) {
            let sliced = turf.lineSliceAlong(
                this.track,
                0,
                this.walkedDistance
            );
